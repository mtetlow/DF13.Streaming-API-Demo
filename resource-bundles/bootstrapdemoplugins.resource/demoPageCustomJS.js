...
function generateTRFromAsset(asset){
	...
	return $j('<tr id="'+asset.Id+'"><td>'+asset.Name+'</td><td>'+asset.Quantity+'</td><td class="account_name" account_id="'+asset.AccountId+'">'+accountName+'</td><td><span class="contact_name">'+contactName+'</span><a class="contact_edit" rel="popover" data-html="true" data-content=\''+contactInput+'\'>(edit)</a></td><td>'+productName+'</td></tr>')[0];
}
...
function connectViaCometD(){
	$j.cometd.init({
		url: window.location.protocol+'//'+window.location.hostname+'/cometd/23.0/',
		requestHeaders: { Authorization: 'OAuth '+apiSessionId}
	});

	$j.cometd.subscribe('/topic/AccountNameChages', function(message) {
		parseAccountNameChangeMessage(message);
	});
}
...
function parseAccountNameChangeMessage(message){
	if(message.data.event.type =='updated' && $j('.account_name[account_id='+message.data.sobject.Id+']').length>0){
		...
		$j('.alert').delay(5000).fadeOut('slow', function (){ $j(this).remove(); });
	}
}

var persistedAssets;
function pollForAssetChanges(callback){
	demoPageAssetController.getAllAssets(function(result,event){
		if(event.status){
